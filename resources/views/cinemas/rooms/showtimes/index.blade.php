    ...
    <div class="min-h-screen bg-gray-50 py-8 px-4 sm:px-6 lg:px-8">
        <div class="max-w-6xl mx-auto">
            <div class="text-center mb-8">
                <h1 class="text-3xl font-bold text-gray-900">Showtimes for Room: {{ $room->name }}</h1>
                <p class="mt-1 text-lg text-gray-600">Cinema: {{ $room->cinema->name }}</p>
                <div class="mt-4">
                    {{-- Link to create a new showtime for this room --}}
                    <a href="{{ route('room.showtime.create', $room->id) }}"
                        class="inline-flex items-center px-4 py-2 bg-indigo-600 border border-transparent rounded-md font-semibold text-white hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                        + Add New Showtime
                    </a>
                    {{-- Link back to the room list for the parent cinema --}}
                            ...
                            @forelse ($showtimes as $showtime)
                                <tr>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        ...
                                        <div class="text-sm text-gray-500">{{ $showtime->start_time->format('Y-m-d H:i') }}</div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        {{-- Format the end time --}}
                                        <div class="text-sm text-gray-500">{{ $showtime->end_time->format('Y-m-d H:i') }}</div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <div class="flex justify-end space-x-2">
                                            {{-- Edit link uses the shallow route 'showtime.edit' --}}
